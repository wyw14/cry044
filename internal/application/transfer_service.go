...
func (s *TransferService) ImportTemplate(ctx context.Context, payload []byte, idempotencyKey string) (domain.StandardTemplate, error) {
	...
	return s.repository.SaveTemplate(ctx, template, 0, "import:"+idempotencyKey)
}
...
func (s *TransferService) PrintReviewSheet(ctx context.Context, batchID string) (string, error) {
	...
	for index, line := range report.Lines {
		results[index] = domain.MaterialResult{MaterialID: line.MaterialID, WeightedScore: line.Score, Passed: line.Passed, Vetoed: line.Vetoed, ReviewerCount: line.ReviewerCount, Disagreements: append([]string(nil), line.Disagreements...)}
	}
