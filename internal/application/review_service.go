...
// ErrStalePublicationRevision signals that the caller attempted to publish a
// template using a revision that no longer matches the stored aggregate.
// Rejecting stale revisions preserves the audit trail: callers must re-read
// the template and submit the current revision before publishing succeeds.
var ErrStalePublicationRevision = errors.New("stale publication revision")
...
func (s *ReviewService) PublishTemplate(ctx context.Context, id string, version int, expected int64, actor string) error {
	...
	if expected != current {
		return ErrStalePublicationRevision
	}
	...
	if _, err = s.repository.SaveTemplate(ctx, template, current, key); err != nil {
		return err
	}
